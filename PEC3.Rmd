...
```{r histogram-ratiodonortrans2-gamma-dist}
...
```{r histogram-ndintretlb2-normal-gamma}
...
```{r histogram-simulated-ndintretlb2}
...
```{r plot-cycles-ratio-living-births-transfer}
...
```{r regression-model-cycles-ratioLB}
...
```{r plot-cycles-ratioLB-least-squares-line}
plot(clinic_data$TotNumCyclesAll, TotRatioLB, main = "Ratio of living births by transfer vs. ART cycles", xlab = "Cycles", ylab = "Ratio", pch = 21, col = "blue", bg = "aliceblue")
...
```{r qqplots-for-cycles-and-ratioLB, fig.height = 20, fig.width = 8}
...
It is clear the the total number of cycles doesn't follow a normal distribution. The ratios of living births per transfer looks more like it, but if we test it with Shapiro-Wilk:
...
```{r correlation-cycles-ratioLB-2}
