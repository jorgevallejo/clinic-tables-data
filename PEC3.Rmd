...
### 7- Final assessment.
Final assessment from source data and analysis:
...
```{r download-2017, eval = FALSE, eval = FALSE}
...
```{r download-previous-years, eval = FALSE}
...
```{r import-data-from-excel, warning=FALSE, eval = FALSE}
library("xlsx")
...
```{r list-codified-variables, eval = FALSE}
...
coded as text, not as numbers. It will be necessary pre-processing the data from the table before we can work with them.
...
```{r process-fractions, warning=FALSE, eval = FALSE}
...
```{r list-of-variables, eval = FALSE}
str(clinic_data, list.len = length(clinic_data))
```
...
*ND_NumTrans*: number of transfers (excluding donors). Transfer is the technique by with an embryo grown in vitro is transferred from cultured to the patient uterus.  
...
```{r summary-total-cycles, eval = FALSE}
...
```{r histograms-transfers-donor, fig.cap="Bar width represents a range of 10 transfers", eval = FALSE}
...
```{r accreditation-table, eval = FALSE}
...
```{r boxplot-intended-retrievals-by-age, eval = FALSE}
...
```{r boxplot-intended-retrievals-live-births, eval = FALSE}
...
```{r summary-numeric-variables, eval = FALSE}
...
```{r log-boxplot-function, eval = FALSE}
...
```{r boxplot-egg-retrieval, fig.height = 15, fig.width = 8, eval = FALSE}
...
```{r boxplot-intended-egg-retrievals, fig.height = 30, fig.width = 8, eval = FALSE}
...
```{r boxplot-transfers, fig.height = 15, fig.width = 8, eval = FALSE}
...
```{r boxplot-transfers-from-donor, fig.height = 15, fig.width = 8, eval = FALSE}
...
```{r boxplot-GFIP, fig.width = 8, eval = FALSE}
...
```{r clinics-high-gestational, eval = FALSE}
...
Clinics: `r #noquote(carrier_clinic$CurrentClinicName1)`  
Cities: `r #noquote(carrier_clinic$CurrentClinicCity)`  
States: `r #noquote(carrier_clinic$CurrentClinicState)`  
...
```{r boxplot-ratio-transfers-by-age, fig.height = 20, fig.width = 8, eval = FALSE}
...
```{r boxplot-cycles-by-age, fig.width = 8, eval = FALSE}
...
```{r boxplot-interruted-cycles, fig.width = 8, eval = FALSE}
...
```{r fertility-preservation-clinic, eval = FALSE}
...
```{r boxplot-cancelled-cycles, fig.width = 8, eval = FALSE}
...
```{r boxplot-stopped-cycles, fig.width = 8, eval = FALSE}
...
```{r boxplot-fertility-preservation, fig.width = 8, eval = FALSE}
...
```{r boxplot-cause-of-cycles, fig.width = 8, eval = FALSE}
...
After that, come ovulatory dysfunction, tubal factor,infertility due to causes not specified and causes unexplained).
The least frequent causes are endometriosis, uterine factor, PGT, recurrent pregnancy loss, causes not related to infertility but not specified, and -almost anecdotal- cycles for using a gestational carrier.
...
```{r table-ratio-transfers-to-ratio-gestational, eval = FALSE}
...
```{r table-clinics-by-city, eval = FALSE}
...
```{r barplot-clinics-by-city, eval = FALSE}
# Most of this code I adapted it from:
# https://stackoverflow.com/questions/10286473/rotating-x-axis-labels-in-r-for-barplot
...
```{r contingency-table-clinics-frequency, eval = FALSE}
clinics_frequency <- table(cities_by_clinic)

clinics_frequency
```

Which informs us that we got `r #clinics_frequency[[1]]` cities with one fertility clinic in them, and one city with `r #noquote(names(clinics_frequency[length(clinics_frequency)]))` clinics in it. 

```{r cities-with-and-without-clinics, eval = FALSE}
clinic_cities <- length(cities_by_clinic)
no_clinic_cities <- 19495 - clinic_cities
```

In total, we have `r #clinic_cities` cities with at least a fertility clinic.

According to the [United States Census Bureau](https://factfinder.census.gov/faces/tableservices/jsf/pages/productview.xhtml?src=bkmk), there are 19,495 cities  (defined as incorporated places, with some exceptions) in the country. Which means that there are `r #no_clinic_cities` cities with no fertility clinic.
...
```{r barplot-clinics-by-city-2, eval = FALSE}
...
```{r barplot-clinics-by-city-logarithmic, eval = FALSE}
...
```{r table-simulated-Poisson, eval = FALSE}
...
In our case:
```{r variance-mean-ratio, eval = FALSE}
...
```{r k-parameter-estimate, eval = FALSE}
...
```{r max-like-estimate-of-k-for-freq, eval = FALSE}
k_for_freq <- kfit(freq)

k_for_freq
```
So, the maximum likelihood of _k_ is this case is `r #k_for_freq`.
...
```{r expected-frequencies-neg-binom, eval = FALSE}
...
```{r barplot-observed-expected, eval = FALSE}
...
```{r histogram-donornumtrans2, eval = FALSE}
...
```{r histogram-ratios-donornumtrans2, eval = FALSE}
...
```{r histogram-ratiodonortrans2-beta-distribution, eval = FALSE}
...
```{r histogram-ratiodonortrans2-gamma-dist, eval = FALSE}
...
```{r histogram-ndintretlb2, eval = FALSE}
...
```{r histogram-ndintretlb2-normal-gamma, eval = FALSE}
...
Given the fit, I would say that the variable ND_IntentRetLB2 follows a **normal distribution** with mean `r #mean(clinic_data$ND_IntentRetLB2, na.rm = TRUE)` and variance `r #var(clinic_data$ND_IntentRetLB2, na.rm = TRUE)`.
...
```{r histogram-simulated-ndintretlb2, eval = FALSE}
...
This histogram looks way smoother than the one built with real data but, all in all, the profile is quite similar. The exception being the few counts of 0.9-1.0 that we found in the real data. I think that the problem here is that, working with data of ratios, we loose the information about the real number of procedures performed at each clinic. That could have an effect on the distribution that best modeled the data.
...
We can use the total number of ART cycles (variable `TotNumCyclesAll`) as a measure of the size of the clinic.  
Getting the number of transfers and the percentage of transfers that resulted in live birth is a bit tricky:

```{r calculate-derived-variables, eval = FALSE}
...
```{r plot-cycles-ratio-living-births-transfer, eval = FALSE}
...
```{r correlation-cycles-ratioLB, eval = FALSE}
...
```{r regression-model-cycles-ratioLB, eval = FALSE}
...
```{r plot-cycles-ratioLB-least-squares-line, eval = FALSE}
plot(clinic_data$TotNumCyclesAll, TotRatioLB, main = "Ratio of living births by transfer vs. ART cycles", xlab = "Cycles", ylab = "Ratio", pch = 21, col = "blue", bg = "aliceblue")

abline(RegModel, col = "red")
```

The correlation coefficient is only 0.008369, which I interpret as a very poor adjust, rending meaningless the values of the model. It may be due to a much greater variance for low number of cycles compared with the variance for high number of cycles; the model used assumes that the variables follow normal distribution and homoscedasticity.

Let's check those assumptions:

```{r qqplots-for-cycles-and-ratioLB, fig.height = 20, fig.width = 8, eval = FALSE}
...
```{r shapiro-ratioLB, eval = FALSE}
shapiro.test(TotRatioLB)
```

With a p-value so low be reject the null hypothesis of normality. We may instead want to check correlation with a test that doesn't assume normality, like Pearson correlation. Which... we already did previously when calculated correlation with `cor()` since the default mode is "pearson".  
The result was `r #cor(clinic_data$TotNumCyclesAll, y=TotRatioLB, use = "complete")`,  
which means that there is not correlation between the variables.  
The significativity of that result is:
```{r correlation-cycles-ratioLB-2, eval = FALSE}
